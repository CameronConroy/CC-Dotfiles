...
Item {
    ...
    // TEMPERATURE LOGIC 
    // -------------------------------------------------------------------------
    property real targetTemp: window.weatherData && window.weatherData.forecast[window.targetWeatherView] ? Number(window.weatherData.forecast[window.targetWeatherView].hourly[window.activeHourInsex !== -1 ? window.activeHourIndex : 0].temp) : 0
    ...
    property int activeHourIndex: {
        if (window.weatherView !== 0 || !window.weatherData || !window.weatherData.forecast || !window.weatherData.forecast[0] || !window.weatherData.forecast[0].hourly) return bestIdx !== -1 ? bestIdx : 0;
            ...
            Item {
                ...
                ColumnLayout {
                    ...
                    RowLayout {
                        ...
                        spacing: window.s(2)
                        Text {
                            text: Qt.formatTime(window.currentTime, "h:mm ap").replace(/ (am|pm)$/i, "")
                        ...
                        Text {
                            text: Qt.formatTime(window.currentTime, "AP")
                            font.family: "JetBrains Mono"
                ...
                Item {
                    ...
                    Repeater {
                        ...
                        delegate: Item {
                            ...
                            property real targetAngleDeg: index * (360 / Math.max(1, mCount))
                            
                            property real orbitOffset: window.globalOrbitAngle * (180 / Math.PI) * -1.5
                            property real osc: 0 
