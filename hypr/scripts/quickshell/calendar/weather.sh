...
get_data() {
    ...
    [ -f "$daily_cache_file" ] && merged_today=$(echo "$merged_today" | jq --slurpfile cache "$daily_cache_file" 'if ($cache | length) > 0 then ($cache[0] + .) | unique_by(.dt) | sort_by(.dt) else . end') || true
    ...
    if [ ! -z "$processed_forecast" ]; then
        ...
        for d in $dates; do
            ...
            for i in $(seq 0 1 $count_slots); do
                ...
                s_temp=$(printf "%.0f" "$raw_s_temp")
                
                s_dt=$(echo "$slot_item" | jq ".dt")
                s_time=$(date -d @$s_dt "+%-I:%M %p")
...
# --- NEW HOURLY MODES FOR TOPBAR ---
    ...
    echo "$(echo "$t" | awk '{printf "%d", int($1 + 0.5)}')°F"
